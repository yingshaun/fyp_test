		...
		while True:
			try:
				...
				if pkt[:5].strip() == Message.HEAD:
					...
					if 'timestamp' in pkt:
						#if self.DEBUG:
						#	printf("pkt,now,diff=%f %f %f"%(pkt['timestamp'], ttt, ttt-pkt['timestamp']), "TIMESTAMP", RED)
				...
				self.t3 += time.clock()-t
				#self.t3 = (self.t3+time.clock()-t)/2 if self.t3!=0 else time.clock()-t
				t = time.clock()
				self.process_pkt(pkt)
				self.t2 += time.clock()-t
				self.t4 = (self.t4+time.clock()-t)/2 if self.t4!=0 else time.clock()-t
			except Exception, e:
